...
def create_app():
    ...
    def fragella_top3():
        ...
        accords = (
            ",".join(data.get("preferred_families", []))
            if data.get("preferred_families")
            else "floral:100,fruity:90,citrus:80"
        )
    ...
    def ai_recommendations():
        ...
        accords = preferred_families or data.get(
            "accords", "floral:100,fruity:90,citrus:80"
        )
    ...
    DB_URL_2 = "postgresql://localhost:5432/pg_db"
    DB_URL = "'postgresql+psycopg2://abeny:abeny2002@localhost:5432/ScentAI'"
    # Database configuration
    database_url = os.getenv("DATABASE_URL") or DB_URL_2
    ...
    def create_user():
        ...
        if existing:
            return (
                jsonify({"error": "User with same username or email already exists"}),
                409,
            )
    ...
    def questionnaire_questions():
        questions = [
            ...
            {
                "id": "preferred_families",
                "label": "Preferred Families (choose up to 3)",
                "type": "select",
                "options": [
                    "Citrus",
                    "Green",
    ...
    def daily_recommendation(user_id):
        ...
        for ans in answers:
            if ans.question_id == "preferred_families" and ans.answer_text:
                preferred_family = (
                    ans.answer_text.split(",")[0] if ans.answer_text else None
                )
        ...
        if preferred_family:
            query = query.filter(
                Perfume.fragrance_family.ilike(f"%{preferred_family}%")
            )
