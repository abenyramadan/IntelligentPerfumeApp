...
class UserProfile(db.Model):
    ...
    )  # Acidic, Neutral, Alkaline, Unknown
    recent_body_state = db.Column(
        db.String(100), nullable=True
    )  # No caffeine/alcohol, Caffeine, Alcohol, Both

    # Climate & Environmental Data
    primary_climate = db.Column(
    ...
    sensitivity_sweetness = db.Column(
        db.String(50), nullable=False
    )  # Low, Medium, High
    sensitivity_projection = db.Column(
        db.String(50), nullable=False
    )  # Low, Medium, High

    # Dose & Application
    number_of_sprays = db.Column(db.Integer, nullable=False)
    preferred_concentration = db.Column(
        db.String(50), nullable=False
    )  # EDT, EDP, Parfum, Extrait
    spray_location = db.Column(
        db.String(50), nullable=False
    )  # Skin only, Clothes only, Mix
    ...
    projection_weight = db.Column(
        db.Float, nullable=False
    )  # Lambda value (0.2, 1.0, 2.0)
