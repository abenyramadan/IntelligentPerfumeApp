...
import { Sparkles, Clock, Wind, Droplets, Star, Heart, TrendingUp, Info } from 'lucide-react'
...
const RecommendationsPage = ({ userId: propUserId }) => {
  ...
  const moods = [
    'Energetic', 'Relaxed', 'Confident', 'Playful', 'Serious', 
    'Romantic', 'Professional', 'Adventurous', 'Calm', 'Creative'
  ]

  const activities = [
    'Office work', 'Gym/Exercise', 'Social event', 'Date night', 
    'Outdoor adventure', 'Shopping', 'Meeting', 'Casual day', 
    'Formal event', 'Travel'
  ]

  const weatherOptions = [
    'Sunny', 'Cloudy', 'Rainy', 'Hot', 'Cold', 'Humid', 'Dry', 'Windy'
  ]
    ...
    return (
      <Card>
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {perfume.fragrance_family && <Badge variant="secondary">{perfume.fragrance_family}</Badge>}
                {perfume.price && <Badge variant="outline">${perfume.price}</Badge>}
                {perfume.longevity_hours && <Badge variant="outline">Longevity: {perfume.longevity_hours}h</Badge>}
                {perfume.sillage && <Badge variant="outline">Sillage: {perfume.sillage}</Badge>}
              </div>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          {perfume.image_url && (
            <img src={perfume.image_url} alt={perfume.name} className="w-32 h-32 object-cover rounded" />
          )}
          {/* Fragrance Notes */}
          <div className="space-y-2">
            {perfume.top_notes?.length > 0 && <div className="text-sm"><strong>Top:</strong> {perfume.top_notes.join(', ')}</div>}
            {perfume.middle_notes?.length > 0 && <div className="text-sm"><strong>Middle:</strong> {perfume.middle_notes.join(', ')}</div>}
            {perfume.base_notes?.length > 0 && <div className="text-sm"><strong>Base:</strong> {perfume.base_notes.join(', ')}</div>}
          </div>
        </CardContent>
      </Card>
    );
  ...
  if (!userId) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center text-gray-600">Please log in to see recommendations.</div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-600">Get personalized perfume suggestions based on your current context</p>
      </div>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Select value={context.mood} onValueChange={(value) => updateContext('mood', value)} placeholder="Select Mood">
                <SelectContent>
            ...
            <div>
              ...
              <Select value={context.activity} onValueChange={(value) => updateContext('activity', value)} placeholder="Select Activity">
                <SelectContent>
      ...
      <Select value={context.primary_climate} onValueChange={(value) => updateContext('primary_climate', value)} placeholder="Select climate">
        <SelectContent>
      ...
      {!loading && recommendations.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">
            Fill in your current context above and get personalized perfume recommendations 
            powered by our advanced AI algorithm.
          </p>
          <Button 
            onClick={getRecommendations}
